    ...
    >
      <div className="w-full max-w-sm bg-[#111] border border-white/5 rounded-xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <p className="text-xs tracking-[0.3em] text-white/60 uppercase">Daily Quote</p>
          <button onClick={onClose} className="text-white/50 hover:text-white/75 text-lg leading-none transition-colors">✕</button>
        </div>
        <div className="px-6 py-6 flex flex-col gap-4">
          <p className="text-sm tracking-[0.05em] text-white/90 leading-relaxed">&ldquo;{quote.q}&rdquo;</p>
          <p className="text-[11px] tracking-[0.2em] text-white/55 uppercase text-right">&mdash; {quote.a}</p>
        </div>
      </div>
    </div>
  );
