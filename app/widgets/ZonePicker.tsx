        ...
        <div className="overflow-y-auto max-h-72 scrollbar-thin">
          {filtered.map((z) => (
            ...
            >
              ...
              <span className={`text-xs font-light tracking-[0.2em] ${input === z.code ? "text-white/80" : "text-white/25"}`}>
                {z.code}
              </span>
            </button>
          ))}
          {filtered.length === 0 && (
            <p className="px-5 py-4 text-xs tracking-[0.1em] text-white/25">No zones found.</p>
          )}
        </div>
