      ...
      <div className="w-full max-w-md bg-[#111] border border-white/10 rounded-xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="px-6 py-8 flex flex-col items-center gap-6 border-b border-white/5 bg-white/[0.02]">
          <div className="text-center w-full">
            <p className="text-sm tracking-[0.1em] text-white/80 font-medium truncate px-4 uppercase">
          ...
          <div className="w-full flex flex-col gap-3">
            <div className="flex justify-between text-[10px] tracking-[0.2em] text-white/30 tabular-nums uppercase">
            ...
            >
              <div
                className="absolute top-0 left-0 h-full transition-all duration-300 ease-out rounded-full bg-white/70"
          ...
          <div className="p-4 flex flex-col gap-1">
            {tracks.map((track) => (
              ...
              >
                ...
                >
                  <p className={`text-xs tracking-[0.15em] truncate uppercase ${track.id === currentTrackId ? "text-white/90" : "text-white/50"}`}>
                    {track.name}
                  </p>
                </button>
                <button
                  onClick={() => onDelete(track.id)}
                  className="opacity-0 group-hover:opacity-100 text-white/20 hover:text-white/60 transition-all p-1"
                >
            ...
            {tracks.length === 0 && (
              <p className="text-center py-8 text-[11px] tracking-[0.2em] text-white/25 uppercase">No files found</p>
            )}
          </div>
        </div>
